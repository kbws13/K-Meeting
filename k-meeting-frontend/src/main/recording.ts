...
const startRecording = (_sender: WebContents, displayId: string | number, mic?: string): void => {
  ...
  let args: string[] = [
    // --- 视频源设定 ---
    '-f',
    'gdigrab', // 使用系统自带 GDI 工具抓取画面
    '-draw_mouse',
    '1', // 1 代表保留展示并录制鼠标光标
    '-framerate',
    '30', // 捕获屏幕使用的预设帧速率 30 fps
    '-offset_x',
    `${bounds.x}`, // 基于抓取设备设定原点截取的 X 轴起始偏置
    '-offset_y',
    '0', // 目前固定为 0，若有副屏特殊情况后期可再动态设置
    '-video_size',
    `${workArea.width}x${workArea.height}`, // 限制捕获并参与重编码的矩形长宽
    '-i',
    'desktop' // 设置桌面为上一步 gdigrab 的信号输入
  ...
  const otherArgs: string[] = [
    // ---------------- 视频输出格式 ----------------
    '-c:v',
    'libx264', // 视频转码器变更为主流的 H.264
    '-preset',
    'ultrafast', // 使用极速模式压缩，减轻录制中占用系统和 CPU 开销
    '-crf',
    '18', // 图像质量系数 18 (较低的 CRF 值会提供极高的视觉保真度)
    '-g',
    '60', // 设置强制发送关键帧的数据间隔 (每 60 帧一个核心关键帧，即 2 秒)
    '-x264-params',
    'nal-hrd=cbr:force-cfr=1', // 基于网络传输兼容性强制固定码率封装

    // ---------------- 音频输出格式 ----------------
    '-c:a',
    'aac', // 采用高级音频编码 AAC 来封装音频数据
    '-b:a',
    '192k', // 给定充足的高品质音频位率 192k
    '-ar',
    '44100', // 取样频率给定为标准的 44100 Hz
    '-ac',
    '2', // 指定立体声声场宽度 (通常为 2。若报错可降级至单声道 1)

    // ---------------- 播放容忍度配置 --------------
    '-pix_fmt',
    'yuv420p', // 设置通用播放器兼容和流媒体兼容性最好的色彩像素排布形式
    ...
    '1', // 流传输包立刻交接刷新
    '-fflags',
    '+genpts', // 迫使底层引擎自主产生音视频对拍的时间戳
    '-max_interleave_delta',
    '0', // 禁止长延迟造成的音视频流切层混用交错
